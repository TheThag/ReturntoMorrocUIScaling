...
def audit(text):
    ...
    if not runs or runs[-1][1] != '3D':
        return 2, ['REVIEW: The newest run is not Phase 3D.']
    run = text[runs[-1].start():]
    samples = list(re.finditer(r'^OWNER INPUT 3D .*$', run, re.M))
    if not samples:
    ...
    for line in sample.splitlines():
        ...
        if line.startswith('UISettings '):
            settings = fields
        elif line.startswith('OwnerStates '):
            if fields.get('blocked', '0') != '0':
                issues.append('Owner-state admission blocked=' + fields['blocked'])
    ...
    lines = ['Phase 3D bounds audit: ' + str(len(checked)) + ' fitted windows checked.']
    ...
    lines.append('Inventory-entry flashing and buff-tooltip placement still require the user result.')
