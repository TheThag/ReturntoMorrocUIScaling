...
def audit(text):
    ...
    if not runs or runs[-1][1] != '3C':
        return 2, ['REVIEW: The newest run is not Phase 3C.']
    run = text[runs[-1].start():]
    samples = list(re.finditer(r'^OWNER INPUT 3C .*$', run, re.M))
    if not samples:
    ...
    lines = ['Phase 3C bounds audit: ' + str(len(checked)) + ' fitted windows checked.']
    ...
    lines.append('F2 overlay operation and buff-tooltip alignment still require the user result.')
